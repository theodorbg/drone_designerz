...
class BladeDesign:
    ...
    def bem_dimensionless(self, tol=1e-6):
        ...
        self.dPower = 0.5 * Nb * omega * rho * c * vrel**2 * (cl * sin(phi_rad) + cd * cos(phi_rad)) * r * dr
        self.dT_be = dC_T_be * (0.5 * rho * A * (omega * R)**2)


    def bem_dimensionless_chat(self, tol=1e-6, max_iter=500):
        """
        Dimensionless BEM for hover.
        lambda_i = v_i / (omega * R)
        """
        C = 0.5
        lambda_i = np.zeros_like(self.r)
        ...
        theta_rad = np.asarray(np.radians(self.twist), dtype=float)
        sigma = self.solidity
        y = self.y
        r = self.r
        R = self.R
        ...
        for iteration in range(max_iter):
            # Flow angle (dimensionless)
            phi_rad = np.arctan2(lambda_i, y)
            aoa_rad = theta_rad - phi_rad
            ...
            # dC_T = sigma/2 * (V_rel/V_tip)^2 * (Cl*cos(phi) - Cd*sin(phi)) * dy
            V_rel_norm = np.sqrt((lambda_i)**2 + y**2)  # Normalized relative velocity
            dC_T_be = sigma / 2 * V_rel_norm**2 * (cl * np.cos(phi_rad) - cd * np.sin(phi_rad)) * dy
            
            # Momentum relation (dimensionless)
            # dC_T = 4 * F * lambda_i * y * dy
            dC_T_mom = 4 * F * lambda_i * y * dy
            
            # Update lambda_i
            lambda_i_new = np.sqrt(np.maximum(dC_T_be / (4 * F * y * dy), 0.0))
            
            # Check convergence
            if np.max(np.abs(lambda_i_new - lambda_i)) < tol:
                lambda_i = lambda_i_new
                break
            
            # Relaxation
            lambda_i = 0.5 * lambda_i + 0.5 * lambda_i_new
        ...
        V_rel_norm = np.sqrt((lambda_i)**2 + y**2)
        dC_T_be = sigma / 2 * V_rel_norm**2 * (cl * np.cos(phi_rad) - cd * np.sin(phi_rad)) * dy
        
        # Convert to dimensional thrust
        V_tip = omega * R
        dT_be = dC_T_be * (0.5 * rho * A * V_tip**2)
        
        # Dimensional power (from blade element)
        v_i = lambda_i * V_tip
        vrel = np.sqrt(v_i**2 + (omega * r)**2)
        dPower = 0.5 * Nb * rho * c * vrel**2 * (cl * np.sin(phi_rad) + cd * np.cos(phi_rad)) * r * dr
